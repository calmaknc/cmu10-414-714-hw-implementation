...
class Op:
    ...
    def compute(self, *args: Tuple[NDArray]): # type: ignore
...
class Value:
    ...
    # dynamic computation
    cached_data: NDArray # type: ignore
    requires_grad: bool
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if node.is_leaf():
            continue
        partial_adjoint_k2i = node.op.gradient_as_tuple(node.grad,node)
        for i,input_node in enumerate(node.inputs):
            if input_node not in node_to_output_grads_list.keys():
                node_to_output_grads_list[input_node] = [partial_adjoint_k2i[i]]
            else:
                node_to_output_grads_list[input_node].append(partial_adjoint_k2i[i])
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node:Value, visited:Dict[Value,bool], topo_order: List[Value]):
    ...
    for in_node in node.inputs:
        if in_node not in visited.keys() or visited[in_node] == False:

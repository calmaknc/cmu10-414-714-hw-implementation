...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size) {
    ...
    // keypoint: conversion between gid(out 1d) to multidim idx, thus locating position in 'a'
    for (int i = shape.size - 1; i >= 0; --i) {
      size_t dim_size = shape.data[i];
      size_t stride = strides.data[i];
      idx += (tmp % dim_size) * stride; // offset of last dim
      tmp /= dim_size; // last second dim -> last dim
    }
...
__global__ void EwiseSetitemKernel(const scalar_t *a, scalar_t *out, size_t size, CudaVec shape,
                                   CudaVec strides, size_t offset) {
  // a is compact and copy a to out
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    // convert gid to multidim, locating in out
    size_t idx = offset;
    size_t tmp = gid;
    for (int i = shape.size - 1; i >= 0; --i) {
      size_t dim_size = shape.data[i];
      size_t stride = strides.data[i];
      idx += (tmp % dim_size) * stride;
      tmp /= dim_size;
    }
    out[idx] = a[gid];
  }
...
__global__ void ScalarSetitemKernel(scalar_t *out, scalar_t val, CudaVec shape,
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray* out, std::vector<int32_t> shape,
  ...
  ScalarSetitemKernel<<<dim.grid,dim.block>>>(out->ptr,val,VecToCuda(shape),VecToCuda(strides),offset,size);
  /// END SOLUTION
}
...
// Elementwise and scalar operations
////////////////////////////////////////////////////////////////////////////////
__global__ void MatmulKernel_WO_SharedMemory(const scalar_t* a, scalar_t* b, scalar_t* out, uint32_t M, uint32_t N, uint32_t P) {
...
__global__ void MatmulKernel_W_SharedMemory(const scalar_t* A, const scalar_t* B, scalar_t* C, int M, int N, int P) {
  ...
  for (int m = 0; m < (N + BLOCK_SIZE - 1) / BLOCK_SIZE; ++m) {
    ...
    // Each thread loads one element of each sub-matrix (cooperative fetching)
    // Boundary checking
    // A M*N
    if (row + blockRow * BLOCK_SIZE < M && m * BLOCK_SIZE + col < N) {
      As[row][col] = A[(row + blockRow * BLOCK_SIZE)*N + m * BLOCK_SIZE + col];
    } else {
      As[row][col] = 0.0f;
    }
    
    // B N*P
    if  (m * BLOCK_SIZE + row < N && blockCol * BLOCK_SIZE + col < P) {
      Bs[row][col] = B[(m * BLOCK_SIZE + row)*P + blockCol * BLOCK_SIZE + col];
    } else {
      Bs[row][col] = 0.0f;
    }
  ...
  if (row + blockRow * BLOCK_SIZE < M && col + blockCol * BLOCK_SIZE < P) {
    C[(row + blockRow * BLOCK_SIZE)*P + col + blockCol * BLOCK_SIZE] = Cvalue;
  } 
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 dimGrid((P + dimBlock.x - 1) / dimBlock.x, (M + dimBlock.y - 1) / dimBlock.y); // note: x,y -> width,height -> col dim, row dim
  MatmulKernel_W_SharedMemory<<<dimGrid,dimBlock>>>(a.ptr,b.ptr,out->ptr,M,N,P);
  /// END SOLUTION
}
...
////////////////////////////////////////////////////////////////////////////////
// why pass cudaarray is wrong ?
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t out_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < out_size) {
    scalar_t max_ele = -INFINITY;
    for (int i = 0; i < reduce_size; i++) {
      max_ele = a[gid*reduce_size + i] > max_ele ? a[gid*reduce_size + i] : max_ele;
    }
    out[gid] = max_ele;
  }
  return;
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t out_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < out_size) {
    scalar_t ele_sum = 0.0;
    for (int i = 0; i < reduce_size; i++) {
      ele_sum += a[gid*reduce_size + i];
    }
    out[gid] = ele_sum;
  }
  return;

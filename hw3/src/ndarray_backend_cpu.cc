...
void EwiseOp(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, BinaryOp op) {
...
template <typename BinaryOp>
void ScalarOp(const AlignedArray& a, scalar_t val, AlignedArray* out, BinaryOp op) {
  for (size_t i = 0; i < a.size; i++) {
...
void ScalarPower(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  ScalarOp(a,val,out,static_cast<scalar_t(*)(scalar_t, scalar_t)>(std::pow));
}

void EwiseMaximum(const AlignedArray& a, const AlignedArray& b, AlignedArray* out) {
  EwiseOp(a,b,out,[](scalar_t x, scalar_t y){return x>y?x:y;});
}

void ScalarMaximum(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  ScalarOp(a,val,out,[](scalar_t x, scalar_t y){return x>y?x:y;});
}
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
  ...
  for(int i = 0; i < m/TILE; i++){
    for(int j = 0; j < p/TILE; j++){
      std::memset(out->ptr+i*p*TILE+j*TILE*TILE,0.0,TILE*TILE*sizeof(scalar_t));
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for(int i = 0; i < out->size; i++) {
    ...
    for(int j = 0; j < reduce_size; j++) {
      max_num = a.ptr[i * reduce_size + j] > max_num ? a.ptr[i * reduce_size + j] : max_num;
    }
    out->ptr[i] = max_num;
  }
  return;

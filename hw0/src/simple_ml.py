...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    return np.average(np.log(np.sum(np.exp(Z),axis=1)) - Z[np.arange(len(y)),y])
    ### END YOUR CODE
...
def softmax_regression_epoch(X:np.ndarray[np.float32], 
    ...
    """
    ### BEGIN YOUR CODE
    # NOTE: X是整个数据集。batch才是batch。要遍历整个数据集
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0,dataset_size,batch):
        ...
        I_y = np.zeros(shape=(batch,W2.shape[1]),dtype=np.uint8)
        I_y[np.arange(batch),y_batch] = 1
        G2 = np.exp(Z1 @ W2) / np.sum(np.exp(Z1 @ W2),axis=1,keepdims=True) - I_y
        G1 = np.where(Z1 > 0,1,0) * (G2 @ W2.T)
        # update parameters
        grad_1 = X_batch.T @ G1 / batch
        grad_2 = Z1.T @ G2 / batch
        W1 -= lr * grad_1
        W2 -= lr * grad_2
    ### END YOUR CODE

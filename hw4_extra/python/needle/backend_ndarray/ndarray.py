...
def cal_stride(shape) -> tuple:
    stride = list(accumulate(reversed(shape),mul))
    return tuple(list(reversed(stride))[1:] + [1])
...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        new_stride = cal_stride(new_shape)
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = [self.shape[ax] for ax in new_axes]

        # permute old stride
        new_stride = [self.strides[ax] for ax in new_axes]
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert(len(self.shape) == len(new_shape)) # dont consider conditions like (4,) -> (4,5)
        for old,new in zip(self.shape,new_shape):
            assert(old == new or old == 1)
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = [(idx.stop - idx.start + idx.step - 1) // idx.step for idx in idxs]
        new_offset = self._offset
        for idx,stride in zip(idxs,self.strides):
            new_offset += idx.start * stride
        new_strides = []
        # step affect strides !!!!!
        new_strides = [idx.step*stride for idx,stride in zip(idxs,self.strides)]
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        # offset = (flip_axis.shape - 1) * self.stride[flip_axis]
        # if multiple axes, treat them as single axis flip and them add them together
        new_offset = 0
    ...
    def pad(self, axes):
        ...
        for i,(left,right) in enumerate(axes):
            shape_pad.append(self.shape[i] + left + right)
            slices.append(slice(left,self.shape[i] + left, 1))
        res = full(shape=tuple(shape_pad),fill_value=0.0 ,device=self.device)
        res[tuple(slices)] = self
        return res
        ### END YOUR SOLUTION

...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        qk = self.matmul(q,k) / math.sqrt(q_dim) 
        mask = self.create_causal_mask(i=queries_len, j=keys_values_len,device=q.device).broadcast_to(qk.shape)
        qk = qk + mask if self.causal else qk
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        k = k.reshape((batch_size*queries_len,k_dim))
        v = v.reshape((batch_size*queries_len,v_dim))

        q_heads = self.q_projection(self.prenorm_q(q)).reshape((batch_size,queries_len,self.num_head,self.dim_head))
        k_heads = self.k_projection(self.prenorm_k(k)).reshape((batch_size,queries_len,self.num_head,self.dim_head))
        v_heads = self.v_projection(self.prenorm_v(v)).reshape((batch_size,queries_len,self.num_head,self.dim_head))
        
        q_heads = q_heads.transpose((1,2))
        k_heads = k_heads.transpose((1,2))
        v_heads = v_heads.transpose((1,2))

        result,self.probs = self.attn(q_heads,k_heads,v_heads)
...
class TransformerLayer(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        assert self.q_features == x_dim
        out = self.dropout(self.attn(x)) + x
        out_reshape = out.reshape((batch_size*seq_len,x_dim))
        out = self.ffn(out_reshape).reshape((batch_size, seq_len, x_dim)) + out
        ### END YOUR SOLUTION
        return out
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.embedding = Embedding(num_embeddings=sequence_len,
                                    embedding_dim=embedding_size,
                                    device=device,
                                    dtype=dtype)
        
        self.transformer_blocks = Sequential(
            *[TransformerLayer(q_features=embedding_size,
                               num_head=num_head,
                               dim_head=dim_head,
    ...
    def forward(
        ...
        t = np.tile(np.arange(seq_len),(batch_size,1))
        t = Tensor(t,device=self.device,dtype=self.dtype)
        time_embedding = self.embedding(t)
        x += time_embedding
